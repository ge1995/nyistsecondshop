...
<div class="container">
    <div class="col-md-9">
        <div class="col-md-12 r1" style="background-color: #e4e4e4;height: 40px">
            <div class="col-md-8" style="margin-top: 5px">
                <B style="color: #c4c4c4; font-size: 20px">商品信息</B>
            </div>
            <div class="col-md-4" style="margin-top: 4px; display: ${collect == 0 ? 'none':''};">
                <B style="display: ${collect == 1 ? 'none':''}; color: #c4c4c4; font-size: 18px;margin-top: 3px; float: right">已收藏</B>
                <a onclick="collect()" class="btn" style="display: ${collect == 2 ? 'none':''}; float: right; margin-top: 0px; padding: 2px">
        ...
        <div class="col-md-12 r2" style="background-color: #f9f9f9; padding: 0px; padding-bottom: 15px">
            <div class="col-md-12" style="margin-top: 40px;">
                        ...
                        <div class="swiper-container">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <img src="<c:url value="${goodInfo.photoUrl}"/>" height="100%" width="85%">
                                </div>
                                <c:forEach var="image" items="${images}">
                                    <div class="swiper-slide">
                                        <img src="<c:url value="${image.url}"/>" height="100%" width="90%">
                                    </div>
                                </c:forEach>
                            </div>
                ...
                <div class="col-md-5">
                    <h3 style="margin-left: 15px"><B>${goodInfo.name}</B></h3>
                    <div style="height: 50px"></div>
                    <div class="col-md-4" style="padding: 0px">
                        <p style="color: #666666; height: 40px"><B>价格</B></p>
                        <p style="color: #666666; height: 40px"><B>卖家</B></p>
                        <p style="color: #666666; height: 40px"><B>商品类型</B></p>
                        <p style="color: #666666; height: 40px"><B>发布时间</B></p>
                    </div>
                    ...
                    <div class="col-md-12" style="padding: 0px">
                        <div onclick="connect()" class="col-md-5 r-c" style="display: ${sessionScope.user.id == goodInfo.goodUser.id ? 'none':''}">
                            <B style="font-size: 20px; color: #e2e2e2">联系卖家</B>
                        </div>
                        <c:choose>
                            <c:when test="${goodInfo.statusId == 1}">
                                <div onclick="buyButton()" class="col-md-5 r-b pull-right" style="display: ${sessionScope.user.id == goodInfo.goodUser.id ? 'none':''}">
                                    <B style="font-size: 20px; color: #e2e2e2">购买</B>
                                </div>
                                <a style="display: ${sessionScope.user.id == goodInfo.goodUser.id ? '':'none'}" href="/nyist-secondshop/goods/userGoodEdit?goodId=${goodInfo.id}">
                    ...
                    <div class="col-md-12" style="margin-top: 50px" align="right">
                        <p><a href="/nyist-secondshop/goods/userGoods?userId=${goodInfo.userId}">查看卖家其他商品</a></p>
                    </div>
                </div>
            </div>
        </div>

        <div class="col-md-12 r1" style="background-color: #e4e4e4; height: 40px; margin-top: 20px;">
            <div class="col-md-6" style="margin-top: 5px">
                <B style="color: #c4c4c4; font-size: 20px">商品描述</B>
            </div>
        </div>
        <div class="col-md-12 r2" style="background-color: #f9f9f9; height: 150px">
        ...
        <div class="col-md-12 r1" style="background-color: #e4e4e4; height: 40px; margin-top: 20px;">
            <div class="col-md-6" style="margin-top: 5px">
                <B style="color: #c4c4c4; font-size: 20px">商品留言</B>
            </div>
        </div>
        <div class="col-md-12 r2" style="padding-top: 15px;background-color: #f9f9f9;">

            <c:choose>
                <c:when test="${reviews.size() == 0}">
                    <div class="col-md-12 r" style="background-color: #ffffff; margin-top: 15px; margin-bottom: 15px">
                        <p></p>
                ...
                <c:otherwise>
                    <c:forEach var="review" items="${reviews}">
                        <div class="col-md-12 r"
                            ...
                            <p onclick="replyFun(${review.id},'${review.fromUser}')"><B
                                    style="color: #2aabd2">${review.fromUser}</B>&nbsp;评论:&nbsp;${review.text}</p>
                            <div class="col-md-10" style="margin-left: 50px">
                                <c:forEach var="reply" items="${review.replys}">
                                    <P style="color: #c4c4c4">时间：${reply.uploadDate}</P>
                                    <p onclick="replyFun(${review.id},'${reply.fromUser}')"><B
                                            style="color: #2aabd2">${reply.fromUser}</B>&nbsp;回复&nbsp;<B
                                            style="color: #2aabd2">${reply.toUser}</B>:&nbsp;${reply.text}</p>
                                </c:forEach>
                            </div>
                            <div id="${review.id}" class="col-md-12" style="margin-bottom: 15px; display: none">
                                <form action="/nyist-secondshop/goods/goodInfo" method="post">
                                    <div class="col-sm-10">
                                        <input id="${review.id}goodId" name="goodId" type="text" value="${goodInfo.id}"
                                               style="display: none">
                                        <input id="${review.id}reviewId" name="reviewId" type="text"
                                               value="${review.id}" style="display: none">
                                        <input id="replyFromUser" name="fromUser" type="text"
                                        ...
                                        <input id="${review.id}replyToUserId" name="toUserId" type="text"
                                               value="${review.fromUserId}" style="display: none">
                                        <input id="${review.id}replyText" name="replyText" type="text"
                                               class="form-control"
                                               placeholder="">
                                    </div>
                                    <button class="btn btn-success; col-sm-1" type="submit">回复</button>
                                    <p class="btn btn-success; col-sm-1" onclick="closeReplyDiv(${review.id})">X</p>
                                </form>
                            </div>
                        </div>
                    </c:forEach>
                ...
                <c:when test="${sessionScope.user != null}">
                    <form action="/nyist-secondshop/goods/goodInfo" method="post">
                        <div class="col-md-12" style="margin-bottom: 15px">
                            <div class="col-sm-11">
                                <input id="goodId" name="goodId" type="text" value="${goodInfo.id}"
                                       style="display: none">
                                <input id="fromUser" name="fromUser" type="text" value="${sessionScope.user.name}"
                                       style="display: none">
                                <input id="fromUserId" name="fromUserId" type="text" value="${sessionScope.user.id}"
                                       style="display: none">
                                <input id="toUserId" name="toUserId" type="text" value="${goodInfo.userId}"
                                       style="display: none">
                                <input id="reviewText" name="reviewText" type="text" class="form-control"
                                       placeholder="留言">
                            </div>
    ...
    <div class="col-md-3 r" style="background-color: #f9f9f9; padding-bottom: 15px;">
        <div class="col-md-12 r" style="text-align: center; margin-top: 10px; background-color: #e4e4e4; padding: 5px">
        ...
        <div class="col-md-12" style="margin-top: 20px;padding-left: 0px;padding-right: 0px">
            ...
            <c:forEach var="good" items="${goods}">
                <a target="_blank" href="goodInfo?goodId=${good.id}">
                    <div class="col-md-12 b" style="padding-left: 0px; padding-right: 0px">
                        <div class="col-md-6" style="padding-top: 15px; padding-bottom: 15px">
                            <img src="<c:url value="${good.photoUrl}"/>" height="100px" ; width="100px">
                        </div>
                        <div class="col-md-6" style="padding-top: 15px; padding-bottom: 15px; height: 130px">
...
<script>
    function collect() {
        ...
        $.ajax({
            type:"POST",
            url:"/nyist-secondshop/collect/insert",
            ...
            success:function (data) {
                if (data === false){
                    ...
                    alert("收藏成功！");
                    $(window).attr('location','/nyist-secondshop/goods/goodInfo?goodId=${goodInfo.id}');
                }
            }
        })
...
<script>
    ...
    function getBuy() {
        if (${sessionScope.user == null}) {
            alert("请先登录！");
            $(window).attr('location','/nyist-secondshop/login');
        } else {
            alert("我们正在为您创建订单！");
            var order = {
                "goodName": "${goodInfo.name}",
            ...
            $.ajax({
                type:"POST",
                url:"/nyist-secondshop/user/order/create",
                contentType: "application/json", //必须这样写
                dataType:"json",
                ...
                success:function (data) {
                    if (data === false){
                        ...
                        $(window).attr('location','/nyist-secondshop/user/orderInfo?orderId=' + data);
                    }
                }
            })
        }
