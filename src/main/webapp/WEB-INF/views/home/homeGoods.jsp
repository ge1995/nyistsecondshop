...
<div class="container">
    <div class="main-content">
        <div class="col-md-2" style="padding-left: 0px; padding-right: 0px;" id="type-bar">
            <div class="col-md-12" align="center" style="padding-left: 0px; padding-right: 0px;" id="type-button">
                <c:forEach var="firstType" items="${firstTypes}">
                    <div style="height: 50px;width: 100%; background-color: #eaeaea; margin-top: 15px;" class="btn-type"
                         align="center;" id="type${firstType.id}"
                         onclick="typeButton(${firstType.id})">
                            <p><h4>${firstType.name}</h4></p>
                    </div>
                    <div class="col-md-12 r" id="${firstType.id}" style="display: none; background-color: #f5f5f5;padding-left: 0px; padding-right: 0px;" align="center">
                        <c:forEach var="secondType" items="${firstType.secondType}">
                            <a class="col-md-12 r-type" href="?secondTypeId=${secondType.id}">
                                <p></p>
                                <p>${secondType.name}</p>
                            </a>
                        </c:forEach>
        ...
        <div class="col-md-10" style="margin-top: 10px; float: right;">
            <c:choose>
                <c:when test="${goods.size() == 0}">
                    <div align="center">
                        <img style="width: 500px; height: 350px" src="<c:url value="/statics/image/photos/default/shouqing.png"/>">
                        <h2>抱歉，暂无此类商品！</h2>
                    </div>
                </c:when>
            </c:choose>
                ...
                <a target="_blank" href="goods/goodInfo?goodId=${good.id}">
                    <div class="c col-md-3">
                        <div class="a col-md-12" style="max-height: 300px;">
                            ...
                            <p style="color: red;text-align: right"><B>价格：${good.prise}￥</B></p>
                        </div>
                    </div>
                </a>
            </c:forEach>
                ...
                <ul class="pagination">
                    ...
                    <c:forEach var="page" begin="1" end="${pages}">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <li>
                                    <a href="?searchText=${param.searchText}&secondTypeId=${param.secondTypeId}&offset=${limit * (page - 1)}">${page}</a>
                                </li>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:choose>
                        <c:when test="${goods.size() >= limit}">
                            <li>
                                <a href="?searchText=${param.searchText}&secondTypeId=${param.secondTypeId}&offset=${offset + limit}">下一页</a>
                            </li>
                        </c:when>
                        <c:otherwise>
