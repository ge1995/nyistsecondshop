...
<div class="container" style="position: relative; transform: translate(0,0)">
    <div class="col-md-9" style="float: left">
        ...
        <div class="col-md-12 r2" style="background-color: #f9f9f9; padding-top: 5px; padding-bottom: 30px; padding-left: 120px; padding-right: 120px; margin-bottom: 20px">
                ...
                <div class="col-md-12" style="margin-bottom: 10px; margin-top: 20px;">
                    <div class="col-md-12" style="padding: 30px; background-color: #f2f2f2">
                        <div class="col-md-5" style="padding-left: 60px; padding-top: 20px">
                            <p style="color: #666666; height: 30px"><B>订单号</B></p>
                            <p style="color: #666666; height: 30px"><B>商品名称</B></p>
                            <p style="color: #666666; height: 30px"><B>卖家</B></p>
                            <p style="color: #666666; height: 30px"><B>买家</B></p>
                            <p style="color: #666666; height: 30px"><B>订单状态</B></p>
                            <p style="color: #666666; height: 30px"><B>交易时间</B></p>
                            <p style="color: #666666; height: 30px"><B>结束时间</B></p>
                        </div>
                        <div id="order" class="col-md-7" style="padding-left: 0px;  padding-top: 20px">
                            <p style="color: #666666; height: 30px"><B>${orderInfo.id}</B></p>
                            <p style="color: #666666; height: 30px"><B>${orderInfo.goodName}</B></p>
                            <p style="color: #666666; height: 30px"><B>${orderInfo.seller}</B></p>
                            <p style="color: #666666; height: 30px"><B>${orderInfo.customer}</B></p>
                            <p style="color: #666666; height: 30px"><B>${orderInfo.statusId == 2 ? "交易中":"交易完成"}</B></p>
                            <p style="color: #666666; height: 30px"><B>${orderInfo.submitDate}</B></p>
                            <p style="color: #666666; height: 30px"><B>${orderInfo.endDate}</B></p>
                        </div>
                    </div>
                    <div class="col-md-12" style="margin-top: 20px">
                        <button class="btn btn-success" onclick="deleteOrder()">删除</button>
                        <button id="success" class="btn btn-primary pull-right" style="display: ${orderInfo.statusId == 2 ? "":"none"}" onclick="selectValue()">完成交易</button>
                        <button id="noSuccess" class="btn btn-primary pull-right" style="display: ${orderInfo.statusId == 3 ? "":"none"}" disabled="true">交易已完成</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div id="re-bar" class="col-md-3 r" style="background-color: #f9f9f9; padding-bottom: 15px;">
        ...
        <div class="col-md-12 r" style="background-color: #ffffff; margin-top: 15px; overflow-y:auto; height: 500px; padding: 5px">
            <c:choose>
                ...
                <c:otherwise>
                    <c:forEach var="order" items="${orders}">
                        <a href="/nyist-secondshop/user/orderInfo?orderId=${order.id}">
...
<script>
    ...
    function deleteOrder() {
        $.getJSON("/nyist-secondshop/user/order/delete/" + ${orderInfo.id}, function (data) {
            if (data == true){
                alert("订单删除成功！");
    ...
    function selectValue() {
        $.getJSON("/nyist-secondshop/user/order/update/status/" + ${orderInfo.id} + "&3", function (data) {
            if (data == false){
                alert("操作失败！");
            } else {
