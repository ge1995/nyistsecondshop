...
import com.nyist.secondshop.models.*;
import com.nyist.secondshop.services.*;
import com.nyist.secondshop.utils.FileCheck;
import com.nyist.secondshop.utils.InfoCheck;
import com.nyist.secondshop.utils.RandomString;
...
public class UserController {
    ...
    @Autowired
    public UserController(GoodService goodService, OrderService orderService, ReviewService reviewService, UserService userService, CollectService collectService) {
    ...
    public String getReplyInfo(@RequestParam(required = false) Integer reviewId,
    ...
    public String postUserEdit(ModelMap model,
                               @Valid User user,
                               HttpSession session,
        ...
        if (!infoCheck.isMobile(user.getMobile())){
            ...
            status = "请输入正确的邮箱！";
        } else if (userService.getUserByMobile(user.getMobile()).getId() != user.getId()) {
            System.out.println(userService.getUserByMobile(user.getMobile()).getId() + " " + user.getId());
            status = "此手机号码已使用！";
        } else if (userService.getUserByEmail(user.getEmail()).getId() != user.getId()) {
            status = "此邮箱已使用！";
        } else {
            if (!photo.isEmpty()){
                ...
                String fileName = user.getId() + randomString.getRandomString(10);
                ...
                String photoUrl = userService.getUserById(user.getId()).getPhotoUrl();
                user.setPhotoUrl(photoUrl);
            }
    ...
    public ResponseEntity editPassword(@RequestBody Password password){
        ...
        String oldPass = DigestUtils.md5DigestAsHex((password.getOldPassword() + user.getCode()).getBytes());
        if (oldPass.equals(user.getPassword())){
            RandomString randomString = new RandomString();
            String code = (randomString.getRandomString(5));
            String md5Pass = DigestUtils.md5DigestAsHex((password.getNewPassword() + code).getBytes());
            Boolean success = userService.updatePassword(md5Pass, code, password.getUserId());
            if (success){
